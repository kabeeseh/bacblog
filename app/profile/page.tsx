...
export default function Home() {
  ...
  const fetchPosts = async () => {
    setLoading(true)
    await axios.get(`/api/posts/profile?page=${page}&limit=5`, {
      headers: {
...
const handleDislike = async (postId: String) => {
  ...
  return <>
    <Nav />
    <div className="">
       <div className="flex justify-center gap-[4vw] mt-[20vh]">
      <h1>Username: {user?.username}</h1>
      <h1>Followers: {user?.followers.length as any}</h1>
      <h1>Following: {user?.following.length as any}</h1>
    </div>
      <InfiniteScroll
