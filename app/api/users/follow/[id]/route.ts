...
export async function POST(req: Request, {params}: { params: {id: String} }) {
  try {
    ...
    if (user1?.id == user2?.id) {
      return new Response("You cant follow yourself", { status: 400 })
    }
    if (user1?.following.includes(user2?.id) || user2?.followers.includes(user1?.id)) return new Response("User already followed")
